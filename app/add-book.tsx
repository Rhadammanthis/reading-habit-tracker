...
import { useActiveBook, useStartBook } from '@/hooks/useBooks';
...
export default function AddBookScreen() {
  ...
  async function confirm() {
    if (!selected) return;
    try {
      await startBook.mutateAsync({ book: selected, goalMinutesPerDay: goal });
      router.replace('/(tabs)');
    } catch (e) {
      setError(
        e instanceof Error && e.message.includes('duplicate')
          ? 'You already have an active book. Finish or remove it first.'
          : e instanceof Error
            ? e.message
            : 'Could not start this book.',
      );
    }
  }
    ...
    return (
      <ScreenContainer edges={['left', 'right', 'bottom']}>
        <View style={{ flex: 1, padding: theme.spacing.xl, gap: theme.spacing.lg }}>
          ...
          <View style={{ flex: 1 }} />
          <Button title="Start reading this book" loading={startBook.isPending} onPress={confirm} />
          <Button
            title="Back"
            variant="ghost"
            onPress={() => {
        ...
        {activeBook ? (
          <Card style={{ backgroundColor: theme.colors.accentMuted, borderColor: theme.colors.accent }}>
            <ThemedText variant="caption">
              You’re currently reading “{activeBook.title}”. Starting a new book requires finishing or
              removing it first.
            </ThemedText>
          </Card>
        ) : null}
