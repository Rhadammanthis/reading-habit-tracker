...
/**
 * The reading-speed test, as a self-contained, navigation-agnostic component.
 * It measures WPM, optionally asks comprehension questions, and persists the
 * result. The host decides what happens when the user leaves the test:
 *  - `onDone` fires when they finish (Continue) or opt out (Skip for now).
 *
 * Used both in first-run onboarding and from Settings to retake the test.
 */
      ...
      <ScrollView contentContainerStyle={{ padding: theme.spacing.xl, gap: theme.spacing.lg }}>
        {phase === 'intro' && (
          <>
            ...
            <Button title={skipLabel} variant="ghost" onPress={onDone} />
          </>
        )}
        ...
        {phase === 'result' && (
          <>
            ...
            <Button title={continueLabel} loading={save.isPending} onPress={onDone} />
          </>
        )}
      </ScrollView>
    </ScreenContainer>
